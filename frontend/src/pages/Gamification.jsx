...
import { FiZap, FiAward, FiTrendingUp, FiStar, FiCheck, FiLock, FiTarget } from 'react-icons/fi';
import Footer from '../components/Footer';
import './Gamification.css';

const DAILY_CHALLENGES = [
  { id: 'study3', title: 'Study for 3 hours', xp: 50, emoji: '📚', description: 'Complete 3 hours of focused study' },
  { id: 'workout', title: 'Workout for 1 hour', xp: 30, emoji: '💪', description: 'Physical exercise for fitness' },
  { id: 'quiz', title: 'Daily Quiz', xp: 40, emoji: '🧠', description: 'Complete the daily knowledge quiz' },
  { id: 'todos', title: 'Complete all tasks', xp: 60, emoji: '✅', description: 'Finish everything on your todo list' },
  { id: 'early', title: 'Wake before 10 AM', xp: 20, emoji: '☀️', description: 'Start your day early' },
  { id: 'timetable', title: 'Follow timetable 80%+', xp: 45, emoji: '📅', description: 'Stick to your daily schedule' },
];

const BADGES = [
  { id: 'nightowl', name: 'Night Owl', emoji: '🌙', condition: 'Study after 11 PM for 7 days', color: '#6C5CE7' },
  { id: 'earlybird', name: 'Early Bird', emoji: '☀️', condition: 'Wake before 7 AM for 7 days', color: '#FDCB6E' },
  { id: 'hardworker', name: 'Hardworker', emoji: '💪', condition: 'Complete all challenges for 5 days', color: '#E17055' },
  { id: 'streakmaster', name: 'Streak Master', emoji: '🔥', condition: '30-day study streak', color: '#00B894' },
  { id: 'quizchamp', name: 'Quiz Champion', emoji: '🧠', condition: 'Score 90%+ on 10 assessments', color: '#0984E3' },
  { id: 'topranker', name: 'Top Ranker', emoji: '🏆', condition: 'Reach top 10 leaderboard', color: '#FDCB6E' },
];
...
export default function Gamification() {
  ...
  useEffect(() => {
    ...
    onValue(ref(database, 'leaderboard'), snap => {
      if (snap.exists()) {
        const data = snap.val();
        const items = Object.entries(data).map(([id, val]) => ({ id, ...val })).sort((a, b) => (b.xp || 0) - (a.xp || 0));
    ...
    <div className="page">
      <div className="container">
        <motion.div className="page-header" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}>
          <h1>🎮 Gamification</h1>
          <p>Complete challenges, earn XP, collect badges, climb the ranks!</p>
        </motion.div>

        {/* Stats Panel */}
        <motion.div className="gam-stats-grid" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }}>
          <div className="gam-stat-card streak-card"><div className="gam-stat-emoji">🔥</div><div className="gam-stat-val">{stats.streak || 0}</div><div className="gam-stat-label">Day Streak</div></div>
          <div className="gam-stat-card xp-card"><div className="gam-stat-emoji"><FiZap /></div><div className="gam-stat-val">{stats.xp || 0}</div><div className="gam-stat-label">Total XP</div></div>
          <div className="gam-stat-card level-card"><div className="gam-stat-emoji"><FiStar /></div><div className="gam-stat-val">Level {stats.level || 1}</div><div className="gam-stat-label">{xpForNextLevel} XP to next</div></div>
          <div className="gam-stat-card badge-card"><div className="gam-stat-emoji"><FiAward /></div><div className="gam-stat-val">{Object.keys(stats.badges || {}).length}</div><div className="gam-stat-label">Badges Earned</div></div>
        </motion.div>
        ...
        <motion.div className="section" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }}>
          ...
          <div className="challenges-grid">
            {DAILY_CHALLENGES.map((ch, i) => (
              <motion.div key={ch.id} className={`challenge-card ${completedChallenges[ch.id] ? 'completed' : ''}`}
        ...
        <motion.div className="section" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.3 }}>
          <div className="section-title"><FiAward /> Badges Collection</div>
          <div className="badges-grid">
            {BADGES.map(badge => {
              const earned = stats.badges && stats.badges[badge.id];
              return (
                <div key={badge.id} className={`badge-card ${earned ? 'earned' : 'locked'}`}>
                  <div className="badge-emoji" style={{ background: earned ? badge.color : '#DFE6E9' }}>{badge.emoji}</div>
                  <h4>{badge.name}</h4>
                  <p>{badge.condition}</p>
              ...
              <tbody>
                {(leaderboard.length > 0 ? leaderboard : [
                  ...
                  <tr key={entry.id} className={entry.id === user?.uid ? 'current-user' : ''}>
                    <td className="rank-cell"><span className={`rank-badge rank-${i + 1}`}>{i === 0 ? '🥇' : i === 1 ? '🥈' : i === 2 ? '🥉' : `#${i + 1}`}</span></td>
        ...
        <motion.div className="section" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.5 }}>
          ...
          <div className="activity-feed">
            {(activityFeed.length > 0 ? activityFeed : [
              ...
              { message: '🏆 Priya earned the Hardworker badge!', timestamp: new Date().toISOString() },
